...
const CayenneElectricTechnicalData: React.FC<TechnicalDataProps> = ({ technicalData }) => {
                    ...
                    <div className="grid grid-cols-1 gap-3 rounded-xl mt-6 w-full">
                        ...
                        <div className="mt-4 flex text-sm flex-col gap-2">
                            <div className="flex justify-between items-center text-sm mb-2">
                                <h3>Max. clearance between ground and water-sensitive parts (normal ride height)</h3>
                                <div className="text-sm flex gap-1 items-center">
                                    <p>{terrainFeatures?.maxClearanceWaterSensitivePartsNormalRideHeight?.value}</p>
                                    <span>{terrainFeatures?.maxClearanceWaterSensitivePartsNormalRideHeight?.unit}</span>
                                </div>
                            </div>

                            <div className="flex justify-between items-center text-sm mb-2">
                                <h3>Max. ground clearance (normal ride height)</h3>
                                <div className="text-sm flex gap-1 items-center">
                                    <p>{terrainFeatures?.maxGroundClearanceNormalRideHeight?.value}</p>
                                    <span>{terrainFeatures?.maxGroundClearanceNormalRideHeight?.unit}</span>
                                </div>
                            </div>

                            <div className="flex justify-between items-center text-sm mb-2">
                                <h3>Max. clearance between ground and water-sensitive parts (off-road ride height)</h3>
                                <div className="text-sm flex gap-1 items-center">
                                    <p>{terrainFeatures?.maxClearanceWaterSensitivePartsOffRoadRideHeight?.value}</p>
                                    <span>{terrainFeatures?.maxClearanceWaterSensitivePartsOffRoadRideHeight?.unit}</span>
                                </div>
                            </div>

                            <div className="flex justify-between items-center text-sm mb-2">
                                <h3>Max. ground clearance (off-road ride height)</h3>
                                <div className="text-sm flex gap-1 items-center">
                                    <p>{terrainFeatures?.maxGroundClearanceOffRoadRideHeight?.value}</p>
                                    <span>{terrainFeatures?.maxGroundClearanceOffRoadRideHeight?.unit}</span>
                                </div>
                            </div>

                            <div className="flex justify-between items-center text-sm mb-2">
                                <h3>Max. clearance between ground and water-sensitive parts (off-road level)</h3>
                                <div className="text-sm flex gap-1 items-center">
                                    <p>{terrainFeatures?.maxClearanceWaterSensitivePartsOffRoadLevel?.value}</p>
                                    <span>{terrainFeatures?.maxClearanceWaterSensitivePartsOffRoadLevel?.unit}</span>
                                </div>
                            </div>

                            <div className="flex justify-between items-center text-sm mb-2">
                                <h3>Max. ground clearance (off-road level)</h3>
                                <div className="text-sm flex gap-1 items-center">
                                    <p>{terrainFeatures?.maxGroundClearanceOffRoadLevel?.value}</p>
                                    <span>{terrainFeatures?.maxGroundClearanceOffRoadLevel?.unit}</span>
                                </div>
                            </div>

                            <div className="flex justify-between items-center text-sm mb-2">
                                <h3>Max. ground clearance (low level)</h3>
                                <div className="text-sm flex gap-1 items-center">
                                    <p>{terrainFeatures?.maxGroundClearanceLowLevel?.value}</p>
                                    <span>{terrainFeatures?.maxGroundClearanceLowLevel?.unit}</span>
                                </div>
                            </div>

                            {terrainFeatures?.breakoverAngleLowLevel && (
                                <div className="flex justify-between items-center text-sm mb-2">
                                    <h3>Breakover angle (low level)</h3>
                                    <div className="text-sm flex gap-1 items-center">
                                        <p>{terrainFeatures.breakoverAngleLowLevel.value}</p>
                                        <span>{terrainFeatures.breakoverAngleLowLevel.unit}</span>
                                    </div>
                                </div>
                            )}
                        ...
                        >
                            <div className="overflow-hidden">
                                ...
                                <div className="grid grid-cols-2 py-2">
                                    ...
                                    <div className="flex items-center gap-1 text-gray-500">
                                        <span className="text-sm font-semibold">
                                            {powerUnit?.overboostPowerWithLaunchControl?.kw}
                                        </span>

                                        <span className="text-sm">kW /</span>

                                        <span className="text-sm">
                                            {powerUnit?.overboostPowerWithLaunchControl?.ps}
                                        </span>
...
export default CayenneElectricTechnicalData;